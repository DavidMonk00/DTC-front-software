...
int main(int argc, char const *argv[]) {
    ...
    for (int i = 0; i < LINK_NUMBER; i++) {
        ...
        std::array<Stub*, PAYLOAD_WIDTH> corrected_stubs = coordinate_corrector.run();
        all_stubs.push_back(corrected_stubs);
        RouterInputReformatting router_formatter(corrected_stubs);
        std::array<uint64_t, 2*PAYLOAD_WIDTH> words_out = router_formatter.run();
        for (int j = 0; j < PAYLOAD_WIDTH; j++) {
